...
for(i in 1:max(c(cd4patient, hivpatient))){
  ...
  if(sum(cd4count[cd4patient==i])!=0 & sum(hivcount[hivpatient==i])!=0 & !is.na(age[age[,1]==i,2])
    ...
    x[[ind]]=matrix(cbind(visitage[hivpatient==i], gender[hivpatient==i], race[hivpatient==i]), ncol=3)
    #x[[ind]]=matrix(cbind(gender[hivpatient==i], race[hivpatient==i]), ncol=3)
    x[[ind]]=matrix(x[[ind]][!c(0, diff(ty[[ind]])<=0),], ncol=3)
...
#h.can=seq(1000, 1000000, length=20)
fold=5
...
X=cbind(1,matrix(unlist(sapply(x, t)), ncol=3, byrow=T))
Y=unlist(y)
...
if(1){
  ...
  Y=data[[2]][data[[5]]]
  Z=data[[3]][data[[5]]]
  x.LOCF=matrix(unlist(sapply(X, t)), ncol=3, byrow=T)
  y.LOCF=unlist(Y)
  z.LOCF=unlist(Z)
  ...
  U1=0;U2=0
  for (i in 1:n) {
    Cov.temp=matrix(c(rep(1, length(Z[[i]])), Z[[i]], X[[i]]), ncol=5, byrow=F)
...
if(1){ # CV
  ...
  for(hh in 1:HH){
    ...
    for(cv in 1:fold){
        ...
        x.center=x0
        y.center=y0
        x.uls=matrix(unlist(sapply(x0, t)), ncol=3, byrow=T)
        t.uls=unlist(ty0)
        y.uls=unlist(y0)
        Ex=list()
        Ey=list()
        ...
        X=matrix(unlist(sapply(x.center, t)), ncol=3, byrow=T)
        Y=unlist(y.center)
...
if(1){
  ...
  y.center=y
  if(1){
    x.uls=matrix(unlist(sapply(x, t)), ncol=3, byrow=T)
    t.uls=unlist(ty)
    y.uls=unlist(y)
    Ex=list()
    Ey=list()
...
if(1){
    ...
    repeat{
      ...
      XX=matrix(unlist(sapply(X.center, t)), ncol=3, byrow=T)
      YY=unlist(Y.center)
...
b.sum=matrix(0, nrow=3, ncol=15)
b.sum[,1]=b.hat.naive[-1]
b.sum[,2]=b.se.naive[-1]
b.sum[,3]=2*pnorm(-abs(b.hat.naive[-1]/b.se.naive[-1]))
b.sum[,4]=b.hat.lvcf
b.sum[,5]=b.se.lvcf
...
plot(unlist(tz)/365.25, unlist(z), cex=2.5, pch='.', ylab='Log CD4 count', xlab='Years')
smooth.z=rep(0, 100)
smooth.tz=seq(min(unlist(tz)), max(unlist(tz)), length.out = 100)
for(i in 1:100){
  smooth.z[i]=sum(K1(smooth.tz[i]-unlist(tz), 500)*unlist(z))/sum(K1(smooth.tz[i]-unlist(tz), 500))
}
lines(smooth.tz/365.25, smooth.z)

plot(tz[[1]]-min(tz[[1]]), z[[1]], type='p', ylim=range(unlist(z)), xlim=c(0,1300), xlab='Days since diagosnis of HIV', ylab='Log(CD4 count)')
for(i in 2:N){
  #if(max(z[[i]])>10) break
  points(tz[[i]]-min(tz[[i]]), z[[i]])
}
