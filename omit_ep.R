...
repeat{
  ...
  if(1){
    ...
    ep=(diag(1,N)-S)%*%y.uls-(diag(1,N)-S)%*%x.uls%*%beta
    #diag(ep^2)
    cc=matrix(0,ncol = dim(S)[1],nrow = dim(S)[1])
    diag(cc)=ep^2
    Vep=t(x.uls)%*%t(diag(1,N)-S)%*%cc%*%(diag(1,N)-S)%*%x.uls
    ...
    se.b.plm.ep[sample]=sqrt(as.numeric(solve(D)%*%Vep%*%solve(D)))
